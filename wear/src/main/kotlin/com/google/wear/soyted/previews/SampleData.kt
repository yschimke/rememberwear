...
object SampleData {
    val localDateTime = LocalDate.of(2021, 2, 18).atTime(12, 35)
    ...
    val taskYesterdayNotCompleted =
        task("1a", localDateTime.toLocalDate().minusDays(1), null)
    val taskYesterdayCompleted =
        task("1a", localDateTime.toLocalDate().minusDays(1), timestamp)
    val taskTodayNotCompleted = task("1a", localDateTime.toLocalDate(), null)
    val taskTodayCompleted =
        task("1a", localDateTime.toLocalDate(), timestamp)
    val taskTomorrowNotCompleted =
        task("1a", localDateTime.toLocalDate().plusDays(1), null)
    ...
    val tileData = RememberWearTileRenderer.TileData(
        today = this.localDateTime.toLocalDate(),
        tasks = taskAndTaskSeries.take(2)
    )
